...
describe('relativeTime', () => {
  ...
  it('formats future timestamps used for grant expiry', () => {
    expect(relativeTime(NOW + 10_000, NOW)).toBe('just now');
    expect(relativeTime(NOW + 2 * 60_000, NOW)).toBe('2 min from now');
    expect(relativeTime(NOW + 3 * 60 * 60_000, NOW)).toBe('3 hr from now');
  });
});
