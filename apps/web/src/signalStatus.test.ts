...
describe('deriveStatus', () => {
  ...
  it("returns loading when status is 'loading' even if last_attempt_at is populated", () => {
    // confirmPlan inserts a signal_status row with status='loading' plus a
    // fresh updatedAt that surfaces as last_attempt_at; without the explicit
    // status check the timestamp would force the signal to 'live'.
